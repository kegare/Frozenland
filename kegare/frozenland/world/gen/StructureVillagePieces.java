...
{
	protected static void registerPieces()
	{
	...
	protected static List getStructureVillageWeightedPieceList(Random random, int size)
	{
		...
		list.add(new StructureVillagePieceWeight(ComponentVillageChurch.class, 20, 1));
		list.add(new StructureVillagePieceWeight(ComponentVillageField.class, 3, MathHelper.getRandomIntegerInRange(random, 1 + size, 4 + size)));
		list.add(new StructureVillagePieceWeight(ComponentVillageField2.class, 3, MathHelper.getRandomIntegerInRange(random, 2 + size, 4 + size * 2)));
		list.add(new StructureVillagePieceWeight(ComponentVillageHall.class, 15, MathHelper.getRandomIntegerInRange(random, 0 + size, 2 + size)));
		list.add(new StructureVillagePieceWeight(ComponentVillageHouse1.class, 20, MathHelper.getRandomIntegerInRange(random, 0 + size, 2 + size)));
		list.add(new StructureVillagePieceWeight(ComponentVillageHouse2.class, 15, 1));
		list.add(new StructureVillagePieceWeight(ComponentVillageHouse3.class, 8, MathHelper.getRandomIntegerInRange(random, 0 + size, 3 + size * 2)));
		list.add(new StructureVillagePieceWeight(ComponentVillageHouse4_Garden.class, 4, MathHelper.getRandomIntegerInRange(random, 2 + size, 4 + size * 2)));
		list.add(new StructureVillagePieceWeight(ComponentVillageWoodHut.class, 3, MathHelper.getRandomIntegerInRange(random, 2 + size, 5 + size * 3)));
	...
	private static int getNextPieceWeight(List list)
	{
		boolean flag = false;
		int i = 0;
		StructureVillagePieceWeight villagePieceWeight;

		for (Iterator iterator = list.iterator(); iterator.hasNext(); i += villagePieceWeight.villagePieceWeight)
		{
			villagePieceWeight = (StructureVillagePieceWeight)iterator.next();
	...
	private static ComponentVillage getNextComponentFromPieceWeight(ComponentVillageStartPiece villageStartPiece, StructureVillagePieceWeight villagePieceWeight, List list, Random random, int x, int y, int z, int coordBaseMode, int type)
	{
	...
	private static ComponentVillage getNextComponent(ComponentVillageStartPiece villageStartPiece, List list, Random random, int x, int y, int z, int coordBaseMode, int type)
	{
		...
		else
		{
			...
			while (var2 < 5)
			{
				...
				while (iterator.hasNext())
				{
					StructureVillagePieceWeight villagePieceWeight = (StructureVillagePieceWeight)iterator.next();
